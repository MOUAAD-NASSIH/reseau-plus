    ...
    return (
        <div className="space-y-4">
            {reviews.map((review) => {
                ...
                const isInstitution = !!(user as any)?.institution || !!(user as any)?.institutionName;
                const isWorker = !!(user as any)?.worker || !!(user as any)?.firstName;

                const name = isInstitution
                    ? (user as any)?.institution?.institutionName || (user as any)?.institutionName
                    : isWorker
                        ? `${(user as any)?.worker?.firstName || (user as any)?.firstName} ${(user as any)?.worker?.lastName || (user as any)?.lastName}`
                        : t("COMMON.UNKNOWN_USER");

                // Robust avatar extraction
                const avatar = isInstitution
                    ? ((user as any)?.institution?.logo || (user as any)?.logo)
                    : ((user as any)?.worker?.profilePicture || (user as any)?.profilePicture);
                                        ...
                                        <div className="text-left md:text-center">
                                            ...
                                            <p className="text-[10px] text-muted-foreground font-medium uppercase tracking-wide md:mb-3">
                                                {isInstitution
                                                    ...
                                                    : (user as any)?.worker?.speciality?.name?.toUpperCase() || t("COMMON.SOCIAL_WORKER")
                                                }
                                            </p>
                                        </div>
                                    </div>
