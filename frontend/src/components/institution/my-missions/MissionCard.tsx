            ...
            <Card className="group relative border-border/50 hover:border-primary/50 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 bg-card overflow-hidden flex flex-col h-full">
                <CardHeader className="p-6 pb-2 space-y-4">
                    ...
                    <div className="space-y-1">
                        <h3 className="font-bold text-lg leading-tight line-clamp-2 text-foreground group-hover:text-primary transition-colors cursor-pointer" onClick={() => navigate(`/institution/missions/${mission.id}`)}>
                            {mission.title}
                        </h3>
        ...
        <div className="group relative flex flex-col md:flex-row items-start md:items-center p-6 bg-card border border-border/60 hover:border-primary/50 rounded-xl shadow-sm hover:shadow-md transition-all duration-300 gap-6">
            ...
            <div className="flex-1 space-y-2 min-w-0 w-full">
                <div className="flex items-center gap-3 mb-1">
                    <h3 className="text-lg font-bold text-foreground group-hover:text-primary transition-colors cursor-pointer truncate" onClick={() => navigate(`/institution/missions/${mission.id}`)}>
                        {mission.title}
                    </h3>
                    <StatusBadge status={mission.status} />
                </div>
                ...
                <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm text-muted-foreground">
                    ...
                    <span className="flex items-center">
                        ...
                        Posted {formatDate(mission.createdAt)}
                    </span>
                    <span className="flex items-center">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 w-full md:w-auto justify-between md:justify-end border-t md:border-t-0 pt-4 md:pt-0 mt-2 md:mt-0">
                <div className="flex items-center gap-3 mr-4 w-full sm:w-auto">
                    ...
                    <div className="flex -space-x-3 hover:space-x-1 transition-all">
                        {[...Array(Math.min(3, applicantsCount))].map((_, i) => (
                            <div key={i} className="h-9 w-9 rounded-full ring-2 ring-background bg-muted flex items-center justify-center overflow-hidden bg-white shadow-sm transition-transform hover:z-10 hover:scale-110">
                                {/* Placeholder Avatar until we have real user data */}
                                <div className="h-full w-full flex items-center justify-center bg-gradient-to-br from-indigo-100 to-purple-100 text-indigo-600 font-bold text-xs">
                                    {["JD", "MK", "AL"][i] || "U" + (i + 1)}
                                </div>
                            </div>
                        ))}
                    ...
                    <div className="flex flex-col">
                        ...
                        {isActiveHiring && <span className="text-[10px] text-emerald-600 font-medium tracking-tight">Active Hiring</span>}
                    </div>
                </div>
