import { Search, Filter, LayoutGrid, List, FileText, XCircle, Clock } from "lucide-react";
      ...
      <div className="flex flex-col md:flex-row gap-4 justify-between bg-card/40 p-4 rounded-2xl border border-border/40 shadow-sm backdrop-blur-md">
        <div className="flex flex-col md:flex-row gap-4 flex-1">
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-2 md:pb-0 no-scrollbar">
            ...
            >
              <SelectTrigger className="h-11 w-[160px] bg-background/50 border-border/50 rounded-xl">
              ...
              <SelectContent className="rounded-xl">
                <SelectItem value="ALL">{t("ADMIN_VALIDATION.DOCUMENTS.ALL_TYPES")}</SelectItem>
                <SelectItem value="PENDING">{t("COMMON.PENDING")}</SelectItem>
                <SelectItem value="APPROVED">{t("PENDING_DOCS.APPROVE")}</SelectItem>
                <SelectItem value="REJECTED">{t("PENDING_DOCS.REJECT")}</SelectItem>
              </SelectContent>
            </Select>

            <Select value={typeFilter} onValueChange={setTypeFilter}>
              <SelectTrigger className="h-11 w-[160px] bg-background/50 border-border/50 rounded-xl">
                <SelectValue placeholder={t("ADMIN_VALIDATION.DOCUMENTS.TYPE_FILTER")} />
              </SelectTrigger>
              <SelectContent className="rounded-xl">
                <SelectItem value="all">{t("ADMIN_VALIDATION.DOCUMENTS.ALL_TYPES")}</SelectItem>
                <SelectItem value="DIPLOMA">{t("PENDING_DOCS.TYPE_DIPLOMA")}</SelectItem>
                <SelectItem value="ID">{t("PENDING_DOCS.TYPE_ID")}</SelectItem>
                <SelectItem value="CV">CV</SelectItem>
                <SelectItem value="LICENSE">{t("PENDING_DOCS.TYPE_LICENSE")}</SelectItem>
              </SelectContent>
            </Select>
          </div>
        </div>
