  ...
  return (
    ...
    >
      ...
      <motion.div variants={reduceMotion ? {} : fadeUpItem}>
        <h2 className="text-xl font-semibold mb-2">{t('AUTH.REGISTER_WORKER.STEP_ACCOUNT.TITLE')}</h2>
        <p className="text-sm text-muted-foreground">
          {t('AUTH.REGISTER_WORKER.STEP_ACCOUNT.SUBTITLE')}
        </p>
      </motion.div>
      ...
      >
        ...
        <div className="space-y-2">
          <Label htmlFor="email" required>{t('AUTH.REGISTER_WORKER.STEP_ACCOUNT.EMAIL_LABEL')}</Label>
          <div className="relative group">
            <Mail className="absolute left-3 top-3.5 h-4 w-4 text-muted-foreground group-hover:text-foreground transition-colors" />
            <Input
              id="email"
              placeholder={t('AUTH.REGISTER_WORKER.STEP_ACCOUNT.EMAIL_PLACEHOLDER')}
              type="email"
              className={cn(
          ...
          {form.formState.errors.email ? (
            ...
            <p className="text-[0.8rem] text-muted-foreground">{t('AUTH.REGISTER_WORKER.STEP_ACCOUNT.EMAIL_HELPER')}</p>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* Password */}
          <div className="space-y-2">
            <Label htmlFor="password" required>{t('AUTH.REGISTER_WORKER.STEP_ACCOUNT.PASSWORD_LABEL')}</Label>
            <div className="relative group">
              <Lock className="absolute left-3 top-3.5 h-4 w-4 text-muted-foreground group-hover:text-foreground transition-colors" />
              <Input
                id="password"
                placeholder={t('AUTH.REGISTER_WORKER.STEP_ACCOUNT.PASSWORD_PLACEHOLDER')}
          ...
          {/* Confirm Password */}
          <div className="space-y-2">
            <Label htmlFor="confirmPassword" required>{t('AUTH.REGISTER_WORKER.STEP_ACCOUNT.CONFIRM_PASSWORD_LABEL')}</Label>
            <div className="relative group">
              <Lock className="absolute left-3 top-3.5 h-4 w-4 text-muted-foreground group-hover:text-foreground transition-colors" />
              <Input
                id="confirmPassword"
                placeholder={t('AUTH.REGISTER_WORKER.STEP_ACCOUNT.CONFIRM_PASSWORD_PLACEHOLDER')}
        ...
        {passwordValue && (
          <div className="p-4 bg-muted/30 rounded-lg border border-border/50 space-y-3">
            <div className="flex items-center justify-between text-xs mb-1">
              <span className="font-medium text-muted-foreground uppercase tracking-wider">{t('AUTH.REGISTER_WORKER.STEP_ACCOUNT.PASSWORD_STRENGTH')}</span>
              <span className={cn("font-bold", strengthColor[passwordStrength].replace("bg-", "text-"))}>
                {t(`AUTH.REGISTER_WORKER.STEP_ACCOUNT.STRENGTH_${strengthLabel[passwordStrength]}`)}
              </span>
            </div>
