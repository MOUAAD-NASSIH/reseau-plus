  ...
  return (
    ...
    >
      ...
      <motion.div variants={reduceMotion ? {} : fadeUpItem}>
        <h2 className="text-xl font-semibold mb-2">{t('AUTH.REGISTER_WORKER.STEP_EXPERIENCE.TITLE')}</h2>
          ...
          >
              ...
              {experiences.length === 0 ? (
                <div className="flex flex-col items-center justify-center p-12 py-16 border-2 border-dashed border-border/50 rounded-2xl bg-secondary/5 text-center space-y-4">
                  ...
                  <div className="space-y-2 max-w-xs mx-auto">
                    <p className="text-sm font-medium text-foreground">{t('AUTH.REGISTER_WORKER.STEP_EXPERIENCE.EMPTY_STATE')}</p>
                  </div>
                  <Button
                    variant="outline"
                  ...
                  {experiences.map((exp, index) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0 pr-12">
                        <h4 className="font-bold text-foreground text-lg truncate group-hover:text-primary transition-colors">{exp.jobTitle}</h4>
                        <p className="text-sm font-medium text-muted-foreground mb-2">{exp.organization}</p>
                        ...
                        {exp.description && (
                          <p className="mt-3 text-sm text-muted-foreground line-clamp-2 leading-relaxed italic">{exp.description}</p>
                        )}
                      </div>
            ...
            <div className="p-5 rounded-2xl bg-primary/3 border border-primary/10 space-y-4">
              <div className="flex items-center gap-3">
                ...
                <h4 className="text-sm font-bold text-primary">{t('AUTH.REGISTER_WORKER.STEP_EXPERIENCE.TIPS_TITLE')}</h4>
              </div>
              <ul className="">
                {[1, 2, 3].map((i) => (
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  <Label htmlFor="jobTitle" required>{t('AUTH.REGISTER_WORKER.STEP_EXPERIENCE.JOB_TITLE_LABEL')}</Label>
                  <div className="relative group">
                    ...
                    <Input
                      id="jobTitle"
                      placeholder={t('AUTH.REGISTER_WORKER.STEP_EXPERIENCE.JOB_TITLE_PLACEHOLDER')}
                      className={cn(
                        "h-11 pl-9 bg-secondary/20 hover:bg-secondary/30 transition-all border-border/50 focus:border-primary/50",
                        form.formState.errors.jobTitle && "border-destructive focus-visible:ring-destructive"
                      )}
                      {...form.register("jobTitle")}
                    />
                  </div>
                  {form.formState.errors.jobTitle && (
                    <p className="text-xs font-medium text-destructive">{form.formState.errors.jobTitle.message}</p>
                  )}
                </div>

                {/* Organization */}
                <div className="space-y-2">
                  <Label htmlFor="organization" required>{t('AUTH.REGISTER_WORKER.STEP_EXPERIENCE.ORGANIZATION_LABEL')}</Label>
                  <div className="relative group">
                    <Building2 className="absolute left-3 top-3.5 h-4 w-4 text-muted-foreground group-hover:text-foreground transition-colors" />
                    <Input
                      id="organization"
                      placeholder={t('AUTH.REGISTER_WORKER.STEP_EXPERIENCE.ORGANIZATION_PLACEHOLDER')}
                      className={cn(
                        "h-11 pl-9 bg-secondary/20 hover:bg-secondary/30 transition-all border-border/50 focus:border-primary/50",
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Start Date */}
                <div className="space-y-2">
                  <Label htmlFor="startDate" required>{t('AUTH.REGISTER_WORKER.STEP_EXPERIENCE.START_DATE_LABEL')}</Label>
                  <div className="relative group">
                    ...
                    <Input
                      ...
                      onChange={(e) => {
                        if (e.target.value) {
                          form.setValue("startDate", new Date(e.target.value), { shouldValidate: true });
                        }
                      }}
                      defaultValue={form.getValues("startDate") ? format(form.getValues("startDate")!, "yyyy-MM-dd") : ""}
                    />
                  </div>
                  {form.formState.errors.startDate && (
                    <p className="text-xs font-medium text-destructive">{form.formState.errors.startDate.message}</p>
                  )}
                </div>

                {/* End Date */}
                <div className="space-y-2">
                  <Label htmlFor="endDate">{t('AUTH.REGISTER_WORKER.STEP_EXPERIENCE.END_DATE_LABEL')}</Label>
                  <div className="relative group">
                    <Calendar className="absolute left-3 top-3.5 h-4 w-4 text-muted-foreground group-hover:text-foreground transition-colors" />
                    <Input
                      id="endDate"
                      ...
                      defaultValue={form.getValues("endDate") ? format(form.getValues("endDate")!, "yyyy-MM-dd") : ""}
                    />
                  </div>
                  {form.formState.errors.endDate && (
                    <p className="text-xs font-medium text-destructive">{form.formState.errors.endDate.message}</p>
                  )}
                </div>
              </div>
              ...
              <div className="flex items-center space-x-2 bg-secondary/10 p-3 rounded-lg border border-border/50">
                ...
                />
                <Label htmlFor="isCurrent" className="text-sm font-semibold cursor-pointer">
              ...
              {/* Description */}
              <div className="space-y-2">
                <Label htmlFor="description">{t('AUTH.REGISTER_WORKER.STEP_EXPERIENCE.DESCRIPTION_LABEL')}</Label>
                <Textarea
                  id="description"
                  placeholder={t('AUTH.REGISTER_WORKER.STEP_EXPERIENCE.DESCRIPTION_PLACEHOLDER')}
                  rows={4}
                  className={cn(
