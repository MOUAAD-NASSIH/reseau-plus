    ...
    >
      ...
      <motion.div variants={reduceMotion ? {} : fadeUpItem}>
        <h2 className="text-xl font-semibold mb-2">
          {t("AUTH.REGISTER_WORKER.STEP_CONFIRM.TITLE")}
        </h2>
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <SectionCard
            ...
            content={
              data.birthDate ? format(new Date(data.birthDate), "PPP") : "—"
            }
          />
        </div>
        ...
        <div className="bg-secondary/10 border border-border/50 rounded-2xl p-5 space-y-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h3 className="font-bold text-sm tracking-tight font-spline text-foreground">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pl-10">
            <div className="space-y-1">
              <Label className="text-[10px] uppercase text-muted-foreground font-bold tracking-widest leading-none bg-secondary/50 px-1.5 py-0.5 rounded-sm">
                {t("AUTH.REGISTER_WORKER.STEP_CONFIRM.SPECIALITY_LABEL")}
              </Label>
              <p className="text-sm font-semibold">{specialityName}</p>
            </div>
            <div className="space-y-1">
              <Label className="text-[10px] uppercase text-muted-foreground font-bold tracking-widest leading-none bg-secondary/50 px-1.5 py-0.5 rounded-sm">
                {t("AUTH.REGISTER_WORKER.STEP_CONFIRM.EXPERIENCE_LABEL")}
              </Label>
              <p className="text-sm font-semibold">
                {data.experienceYears}{" "}
                {t("AUTH.REGISTER_WORKER.STEP_CONFIRM.YEARS")}
              </p>
            </div>
            <div className="md:col-span-2 space-y-1">
              <Label className="text-[10px] uppercase text-muted-foreground font-bold tracking-widest leading-none bg-secondary/50 px-1.5 py-0.5 rounded-sm">
                {t("AUTH.REGISTER_WORKER.STEP_CONFIRM.DOMAINS_LABEL")}
              </Label>
              <div className="flex flex-wrap gap-1.5 pt-1">
                {selectedDomains.map((d, i) => (
                  <span
                    key={i}
                    className="text-[11px] font-bold bg-primary/10 text-primary px-2 py-0.5 rounded-full ring-1 ring-primary/20"
                  >
                    {d}
                  </span>
                ))}
            ...
            {data.bio && (
              <div className="md:col-span-2 space-y-1 pt-2 border-t border-border/30">
                <Label className="text-[10px] uppercase text-muted-foreground font-bold tracking-widest leading-none">
                  {t("AUTH.REGISTER_WORKER.STEP_CONFIRM.BIO_LABEL")}
                </Label>
          ...
          >
            ...
            <div className="space-y-1">
              <Label
                ...
                className="text-sm font-bold cursor-pointer leading-tight block"
              >
  ...
  return (
    <div className="bg-secondary/10 border border-border/50 rounded-2xl p-4 flex items-center gap-4 group hover:border-primary/30 transition-colors">
      ...
      <div className="flex-1 min-w-0">
        <h4 className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest font-spline">
          {title}
        </h4>
        <p className="text-sm font-bold text-foreground truncate font-sans">
          {content}
        </p>
      </div>
    </div>
...
function AlertCircle(props: any) {
  return (
    <ShieldCheck
      {...props}
      className={cn(props.className, "text-destructive")}
    />
  );
}
