 ...
 * Includes Institution details in reports and receipts
 * ==========================================
 */

/**
 * Format admin payment data for export (includes Institution)
 */
export const formatAdminPaymentDataForExport = (payments: any[]): (PaymentExportData & { institutionName: string })[] => {
    return payments.map(payment => {
        const assignment = payment.assignment || payment.missionAssignment;
        const mission = assignment?.mission;
        const worker = assignment?.worker;
        const institution = mission?.institution || payment.institution;
...
/**
 * Export Admin Payemnts to Excel (Includes Institution Column)
 */
export const exportAdminReportToExcel = (payments: any[]) => {
    const formattedData = formatAdminPaymentDataForExport(payments);
    ...
    const headerData = [
        ...
        [`Total Payments: ${formattedData.length}`],
        [], // Empty row
        ['Mission', 'Status', 'Institution', 'Worker', 'Speciality', 'Date', 'Amount (MAD)', 'Platform Fee (MAD)']
    ];
    ...
    const sheetData = [...headerData, ...paymentRows, [], totalsRow];
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Admin Payments');
    const filename = `Admin_Payment_Report_${format(new Date(), 'yyyy-MM-dd_HHmm')}.xlsx`;
    XLSX.writeFile(wb, filename);
};

/**
 * Export Admin Payments to CSV
 */
export const exportAdminReportToCSV = (payments: any[]) => {
    const formattedData = formatAdminPaymentDataForExport(payments);

    const headers = ['Mission', 'Status', 'Institution', 'Worker', 'Date', 'Amount (MAD)', 'Platform Fee (MAD)'];
    const rows = formattedData.map(p => [
        `"${p.missionTitle}"`, // Quote mission title
        p.status,
        `"${p.institutionName}"`, // Quote institution name
        p.workerName,
        p.date,
        p.amount,
        p.platformFee || 0
    ...
    const url = URL.createObjectURL(blob);
    link.setAttribute('href', url);
    link.setAttribute('download', `Admin_Payment_Report_${format(new Date(), 'yyyy-MM-dd_HHmm')}.csv`);
    link.style.visibility = 'hidden';
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
...
/**
 * Generate Admin Receipt PDF (Includes Institution Details)
 */
export const exportAdminReceiptToPDF = (payment: any) => {
    const assignment = payment.assignment || payment.missionAssignment;
    const mission = assignment?.mission;
    const worker = assignment?.worker;
    const institution = mission?.institution || payment.institution;
        ...
        <body>
            <div class="receipt-container">
                    ...
                    <div class="section">
                        ...
                        <div class="info-grid">
                            <div class="info-item">
                                ...
                                <div class="info-value">${institution?.institutionName || 'N/A'}</div>
                            </div>
                            <div class="info-item">
                                <div class="info-label">City</div>
                                <div class="info-value">${institution?.city || 'N/A'}</div>
                            </div>
                            <div class="info-item">
                                <div class="info-label">Mission</div>
                                <div class="info-value">${mission?.title || `Assignment #${payment.missionAssignmentId}`}</div>
                ...
                <div class="footer">
                    ...
                    <p>Generated by Réseau Admin Portal</p>
                    <p class="timestamp">Printed on ${format(new Date(), 'dd/MM/yyyy HH:mm')}</p>
                </div>
            </div>
        </body>
        </html>
